...
class ReviewAudit1 extends Page implements HasForms, HasActions
{
    use InteractsWithForms, InteractsWithActions;

    protected static string $resource = ReviewIndikatorResource::class;
    protected static string $view = 'filament.resources.review-indikator-resource.pages.review-auditor1';
   ...
   protected function validateUserAccess(): void
{
    ...
    // Jika super_admin, izinkan tanpa validasi lainnya
    if ($user->hasRole('super_admin')) {
        return;
    }

    $isAuditor1 = $this->assignment->assignAuditors &&
        $this->assignment->assignAuditors->auditor1 == $user->id;
    
    if (!$isAuditor1) {
        abort(403, 'Anda bukan auditor 1 untuk assignment ini.');
    }
                                                    ...
                                                    ->schema([
                                                        ...
                                                        Forms\Components\Repeater::make('isi_indikators')
                                                                    ...
                                                                    ->schema([
                                                                        ...
                                                                        Forms\Components\Section::make('Review Auditor 2')
                                                                            ->schema([
                                                                                Forms\Components\Grid::make(2)
                                                                                    ->schema([
                                                                                        Forms\Components\Placeholder::make('review_auditor2_status_temuan_display')
                                                                                            ->label('Temuan Auditor 2')
                                                                                            ->content(function (Forms\Get $get) {
                                                                                                $temuan = $get('review_auditor2_status_temuan');
                                                                                                $status = $get('review_auditor2_status');
                                                                                        ...
                                                                                        Forms\Components\Placeholder::make('review_auditor2_reviewed_at_display')
                                                                                            ...
                                                                                            ->content(
                                                                                                fn(Forms\Get $get) =>
                                                                                                $get('review_auditor2_reviewed_at') ?
                                                                                ...
                                                                                Forms\Components\Placeholder::make('review_auditor2_hasil_pengamatan_display')
                                                                                    ->label('Hasil Pengamatan Auditor 2')
                                                                                    ->content(function (Forms\Get $get) {
                                                                                        $pengamatan = $get('review_auditor2_hasil_pengamatan');
                                                                                        $status = $get('review_auditor2_status');
                                                                                ...
                                                                                Forms\Components\Placeholder::make('conflict_indicator')
                                                                                    ...
                                                                                    ->content(function (Forms\Get $get) {
                                                                                        ...
                                                                                        if ($auditor1Status === 'reviewed' && $auditor2Status === 'reviewed') {
                                                                                            if ($auditor1Temuan !== $auditor2Temuan) {
                                                                                                return new \Illuminate\Support\HtmlString(
                                                                        ...
                                                                        Forms\Components\Section::make('Review Auditor 1')
                                                                            ->schema([
                                                                                Forms\Components\Grid::make(2)
                                                                                    ->schema([
                                                                                        Forms\Components\Select::make('review_auditor1_status_temuan')
                                                                                            ->label('Temuan')
                                                                                            ->options([
                                                                                                'sesuai' => 'Sesuai',
                                                                                        ...
                                                                                        Forms\Components\Placeholder::make('review_auditor1_status_display')
                                                                                            ->label('Status Review')
                                                                                            ->content(
                                                                                                fn(Forms\Get $get) =>
                                                                                                $get('review_auditor1_status') === 'reviewed' ?
                                                                                                    '✅ Sudah direview pada ' . ($get('review_auditor1_reviewed_at') ?
                                                                                                        \Carbon\Carbon::parse($get('review_auditor1_reviewed_at'))->format('d/m/Y H:i') : '') :
                                                                                                    '⏳ Belum direview'
                                                                                            ),
                                                                                    ]),

                                                                                Forms\Components\Textarea::make('review_auditor1_hasil_pengamatan')
                                                                                    ...
                                                                                    ->rows(4)
                                                                                    ->required(fn(Forms\Get $get) => $get('review_auditor1_status_temuan') === 'tidak_sesuai')
                                                                                    ->columnSpanFull(),
                                                                            ])
                                                                            ->collapsible()
                                                                            ->collapsed(false)
                                                            ...
                                                            ->itemLabel(function (array $state): ?string {
                                                                ...
                                                                $status = $state['review_auditor1_status'] ?? 'pending';
                ...
                foreach ($standarGroup['sub_standars'] as $subStandar) {
                    foreach ($subStandar['isi_indikators'] as $isiIndikator) {
                        if (!empty($isiIndikator['pengisian_id'])) {
                            \App\Models\ReviewAuditor1::updateOrCreate(
                                [
                                    'pengisian_indikators_id' => $isiIndikator['pengisian_id'],
                                ],
                                [
                                    'status_temuan' => $isiIndikator['review_auditor1_status_temuan'] ?? null,
                                    'hasil_pengamatan' => $isiIndikator['review_auditor1_hasil_pengamatan'] ?? null,
                                    'status' => 'reviewed',
    ...
    {
        return 'Review Auditor 1 - ' . ($this->assignment->unitIndikator->unit->nama_unit ?? 'Unknown');
    }
